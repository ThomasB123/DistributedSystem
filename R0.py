...
class server(object):
    ...
    def update(self, x, out): # update self, then send updates to other servers
        ...
        response.append(out)
        try:
            R1.getUpdate("R0", requestID, response)
        except:
            pass
        try:
            R2.getUpdate("R0", requestID, response)
        except:
            pass
...
if __name__ == "__main__":
    ...
    ns.register("R0", uri) # register the object with a name in the name server
    ...
    R1 = Pyro4.Proxy("PYRONAME:R1") # another server
